...
texte = open("../naiveBayesModel/dataForPrediction/lemmesDatasetPrediction.txt","r")
...
if re.match(r"[a-zA-Z0-9_-]+\.arff", nomFichierSortie):
	nomFichierSortie = "../naiveBayesModel/dataForPredictionPreprocessed/"+nomFichierSortie
	#On applique les filtres de weka sur le fichier arff (par exemple StringToWordVector en mode batch -> voir docs_et_liens_utiles/liens_utiles.txt)
	# -b : mode batch
	...
	os.system("java -Xmx2048M weka.filters.unsupervised.attribute.StringToWordVector -W 100000 -L -b -i ../naiveBayesModel/dataForPrediction/fichierUtilePourPretraitementDataPrediction.arff -o taoctraotrtoazxio.arff -r skjchgjkbgsmyfbcg.arff -s "+nomFichierSortie+" -stopwords-handler \"weka.core.stopwords.WordsFromFile -stopwords stopwords.txt\" -tokenizer \"weka.core.tokenizers.NGramTokenizer -max 3 -min 1 \"")
